...
//제너릭 클래스 : 클래스를 선언 할때 타입 변수는 선언 <T> <<정의 >> ,  객체를 생성시 T 에 적횽할 타입을 지정 

//제너릭 메소드 : 일반 클래스 배부의 메소드 선언시 제너릭 타입 변수를 사용. 

class Genericmethod3 {   //일반 클래스 
	...
	public <T> boolean method2 (T t1, T t2)	{  //제너릭 메소드 : 1개
		return t1.equals(t2); 		// 두값을 비교 true, false 리턴 
									// 기본 자료형일때 는 값을 비교. 참조 자료형일때는 객체의 주소를 비교, 
									// String 일 경우 , 값을 비교. Interger, Double equals가 재정의 되어 있다. 
...
public class GernericMethod {
	public static void main(String[] args) {
		...
		Genericmethod3 gm = new Genericmethod3(); 
		String str1 = gm.<String>method1("안녕");
		String str2 = gm.method1("안녕");			//타입을 알수 있는 경우 생략 가능 (매개변수로 값이 들어갈때 생략가능 )
		...
		Boolean d1 = gm.<Double>method2(2.55, 2.55);  //리던 받을 타입이 Boolean 
		Boolean d2 = gm.<Double>method2(2.55, 2.45);  //리던 받을 타입이 Boolean 
		System.out.println(d1);
		System.out.println(d2);
		System.out.println("=========================");
		
		gm.<String, Integer>method3("국어", 90); 
		gm.method3("영어", 80);         // 매개변수로 인풋 되는 데이터 타입을 처정할 수 있는 경우 생략 가능 
